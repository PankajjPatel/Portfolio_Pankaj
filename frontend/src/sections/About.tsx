...
export const About: React.FC = () => {
  return (
    <section id="about" className="relative py-24 px-6 overflow-hidden bg-black/40">
      {/* Background elements */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {stats.map((stat, idx) => (
              <motion.div
                ...
                className="glass-panel p-6 rounded-2xl flex flex-col gap-4 border border-white/5 hover:border-accentViolet/30 transition-all duration-300 shadow-[0_4px_30px_rgba(0,0,0,0.2)]"
              >
                <span className="text-4xl font-extrabold text-transparent bg-clip-text bg-accent-gradient select-none">
                  {stat.id}
                </span>
                <p className="text-sm font-semibold tracking-wide text-gray-200 uppercase leading-snug">
                  {stat.label}
                </p>
              </motion.div>
            ))}

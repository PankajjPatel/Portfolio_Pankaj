...
export const MarqueeShowcase: React.FC = () => {
  ...
  return (
    <section 
      ref={containerRef}
      className="relative py-24 bg-[#0C0C0C] border-b border-white/5 overflow-hidden select-none"
    >
        ...
        <div className="flex overflow-hidden w-full">
          ...
          >
            ...
            {[...row1Items, ...row1Items].map((item, idx) => (
              <div 
                key={idx}
                className="w-[280px] sm:w-[360px] aspect-[16/10] shrink-0 rounded-2xl border border-white/5 bg-[#111827]/80 overflow-hidden relative group cursor-pointer shadow-lg hover:border-primaryBlue/30 transition-all duration-300"
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end p-5">
                  <div className="flex items-center gap-3">
                    <div className="p-2 rounded-lg bg-black/50 border border-white/10 text-white backdrop-blur-md">
                      {item.icon}
                    </div>
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{item.category}</span>
                      <h3 className="text-sm sm:text-base font-extrabold text-white font-kanit uppercase">{item.title}</h3>
                    </div>
                  </div>
                </div>
              </div>
